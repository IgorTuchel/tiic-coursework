import Roles from "../models/appdb/roles";
import User from "../models/appdb/users";
...
export async function handlerLogin(req, res) {
  try {
    ...
    if (!dbUser) {
      throw new BadRequestError(req, "Invalid email or password");
    }
    ...
    if (!dbRole) {
      throw new InternalServerError(req, "User role not found");
    }

    // MFA handle
    if (dbUser.mfaEnabled || dbRole.mfaRequired) {
      if (!mfaCode) {
        throw new BadRequestError(req, "MFA code is required");
      }
      const { success, data } = await handleMFA(mfaCode, dbUser.userID);
      if (!success) {
        if (data.code === StatusCodes.FORBIDDEN) {
          throw new ForbiddenError(
            req,
            data.message,
