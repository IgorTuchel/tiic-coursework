/**
 * @file handlerDeactivateAccount.js
 * @description Handler for deactivating a user account.
 * @module handlers/users/handlerDeactivateAccount.js
 */
...
/**
 * Deactivates a user account by updating the user's status to "deactivated".
 * @async
 * @function handlerDeactivateAccount
    ...
    const deactivatedStatus = await getUserStatuses()
        .then((res) => {
            ...
            return res.data.find(
                (status) => status.statusName === "deactivated",
            );
        })
        .catch((err) => {
