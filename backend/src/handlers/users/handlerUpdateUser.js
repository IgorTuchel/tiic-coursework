 ...
 * @description Handler for updating a user's information.
 * @module handlers/users/handlerUpdateUser.js
 */
...
export async function handlerUpdateUser(req, res) {
    ...
    if (statusID) {
        ...
        if (statusID != user.data.statusID) {
            const usersCurrentStatus = await getUserStatusByID(
                user.data.statusID,
            );
    ...
    await User.update(
        {
            ...
            mfaEnabled:
                mfaEnabled !== undefined ? mfaEnabled : user.data.mfaEnabled,
